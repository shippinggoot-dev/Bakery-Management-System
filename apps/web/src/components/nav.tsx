      ...
      <header className="fixed inset-x-0 top-0 z-40 bg-rose-50 border-b border-rose-100">
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-rose-100 bg-rose-50 py-2 px-3 flex flex-col gap-1 max-h-[75vh] overflow-y-auto">
            ...
            {isLoggedIn && !isAnonymous && (
              <>
                <Link href="/settings" className="px-4 py-2.5 rounded-lg text-sm text-brand-500 hover:bg-brand-100">{t("settings")}</Link>
                <button onClick={handleSignOut} className="px-4 py-2.5 rounded-lg text-sm text-brand-400 hover:bg-brand-100 text-left">{t("signOut")}</button>
              </>
            )}
          </div>
        )}
