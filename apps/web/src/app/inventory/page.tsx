...
const STATUS_CONFIG: Record<StockStatus, { label: string; bar: string; badge: string; row: string }> = {
  ok:       { label: "OK",       bar: "bg-emerald-500", badge: "bg-emerald-900/50 text-emerald-300 border-emerald-700", row: "" },
  low:      { label: "Low",      bar: "bg-amber-500",   badge: "bg-amber-900/50 text-amber-300 border-amber-700",     row: "bg-amber-950/10" },
  critical: { label: "Critical", bar: "bg-red-500",     badge: "bg-red-900/50 text-red-300 border-red-700",           row: "bg-red-950/20" },
  out:      { label: "Out",      bar: "bg-gray-600",    badge: "bg-gray-800 text-gray-400 border-gray-600",           row: "bg-red-950/30" },
};
...
function BarFill({ current, par }: { current: number; par: number | null }) {
  ...
  const status: StockStatus = pct <= 0 ? "out" : pct < 50 ? "low" : pct < 100 ? "ok" : "ok";
  return (
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 pb-8">
      ...
      <div className="grid grid-cols-2 gap-3 px-4 pt-4 lg:px-8 lg:grid-cols-4">
        <Link href="/inventory/waste" className="rounded-xl border border-gray-800 bg-gray-900 p-4 hover:border-gray-600 transition-colors">
          ...
          <div className="text-sm font-semibold text-gray-200">Log waste</div>
          <div className="text-xs text-gray-500">Record spoilage or damage</div>
        </Link>
        <Link href="/inventory/report" className="rounded-xl border border-gray-800 bg-gray-900 p-4 hover:border-gray-600 transition-colors">
          ...
          <div className="text-sm font-semibold text-gray-200">Waste report</div>
        ...
        <Link href="/purchase-orders" className="rounded-xl border border-gray-800 bg-gray-900 p-4 hover:border-gray-600 transition-colors">
          <div className="text-2xl mb-1">📦</div>
          <div className="text-sm font-semibold text-gray-200">Orders</div>
          <div className="text-xs text-gray-500">View draft reorders</div>
        </Link>
        <Link href="/inventory/produce" className="rounded-xl border border-gray-800 bg-gray-900 p-4 hover:border-gray-600 transition-colors">
          <div className="text-2xl mb-1">🧁</div>
          <div className="text-sm font-semibold text-gray-200">Produce</div>
          <div className="text-xs text-gray-500">Deduct for a batch</div>
        </Link>
      </div>
      ...
      <div className="px-4 pt-4 lg:px-8 space-y-2">
        {isLoading && (
          <div className="text-center text-gray-600 py-12">Loading stock levels…</div>
        )}
        ...
        {filtered.map((item) => {
          ...
          return (
            ...
            >
              ...
              >
                {/* Status dot */}
                <div className={`w-3 h-3 rounded-full flex-shrink-0 ${cfg.bar}`} />

                {/* Name + bar */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-sm truncate">{item.name}</span>
                    <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${cfg.badge}`}>
                      {cfg.label}
                    </span>
              ...
              {isExpanded && (
                <div className="border-t border-gray-800 px-4 py-3 space-y-3">
                  <div className="flex gap-6 text-xs text-gray-500">
                    {item.parLevel && (
                      <span>Par: <span className="text-gray-300">{item.parLevel} {item.unit}</span></span>
                    )}
                    {item.reorderPoint && (
                      <span>Reorder at: <span className="text-gray-300">{item.reorderPoint} {item.unit}</span></span>
                    )}
                  </div>

                  {item.lots.length > 0 ? (
                    <div className="space-y-1">
                      <div className="text-[10px] font-semibold text-gray-600 uppercase tracking-wider mb-1">Active lots (FEFO order)</div>
                      {item.lots.map((lot) => {
                        ...
                        return (
                          <div key={lot.id} className="flex items-center justify-between bg-gray-800/60 rounded-lg px-3 py-2 text-xs">
                            <div className="text-gray-400">
                              {lot.lotNumber ? <span className="font-mono text-gray-300">{lot.lotNumber}</span> : <span className="italic text-gray-600">No lot #</span>}
                            </div>
                    ...
                    <p className="text-xs text-gray-600 italic">No active lots — receive a delivery to add stock.</p>
                  )}
