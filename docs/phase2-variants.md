...
1. **Per-variant ingredient changes — Resolved: shared recipe (a).** ~~A flavour swap doesn't change the underlying recipe drastically enough to justify a different recipe per variant. Fondant / filling changes are handled as **paid addons** on top of a single base recipe (the existing `cake_addons` model). Consequence: dropped the `recipeId` override column from the `premade_cake_variants` schema in §4.1.~~
   ...
   **Reversed 2026-06-10.** When we sat down to import Sucre's real catalog we found the flavour axis names cakes the bakery actually bakes from different sponge recipes — `Cookies & Cream`, `Midnight Cherry`, `Citrus Dream`, `Chocolux`, etc. are not fondant/filling swaps on a shared base; they're independent bakes. Re-added the optional per-variant `recipeId` override on `premade_cake_variants` via migration `2026-06-10_variant_recipe_override.sql`. Null still means "inherit the parent cake's recipeId" (the original design); a non-null value wins. Production scheduler and webhook/import auto-plan both updated to dereference `variant.recipeId ?? cake.recipeId`. Modal and cake editor expose a recipe dropdown per variant with "Inherit from cake" as the default.
